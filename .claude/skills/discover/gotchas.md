...
- Zotero access uses `zotero-cli` (Bash) first — preferred for Claude Code because it's lower token cost. MCP tools (`mcp__zotero-mcp__*`) are the fallback. Both require the Zotero desktop app to be open with local API enabled (`Tools → Developer → Allow other applications to communicate with Zotero`).
...
- Optional: after `/discover lit`, newly found papers can be added back to Zotero via `zotero-cli add doi [DOI]` or `mcp__zotero-mcp__zotero_add_by_doi`. Only do this if the user asks — don't auto-add.
- The Librarian/explorer agents (and their critics) may also have direct `mcp__zotero-mcp__*` tools in their own toolset — see `.claude/references/zotero-search-protocol.md`. This is separate from the CLI/export bridge above and both can coexist safely.
