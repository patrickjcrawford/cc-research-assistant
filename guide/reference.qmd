...
| `/talk` | `create`, `audit`, `compile`; formats: `job-market`, `seminar`, `short`, `lightning`; `--quarto` | Presentations (Beamer or Quarto RevealJS) |
...
This is the most complex skill. Routing depends on file type and flags.
...
**Syntax:** `/talk [mode] [format] [--quarto] [file]`
...
4. For `compile`: run latexmk (Beamer) or `quarto render` (Quarto).

**Modes / Flags:**

| Mode | Description |
|------|-------------|
| `create job-market` | 40--50 slides, 45--60 min, full story |
...
| `create [format] --quarto` | Quarto RevealJS instead of Beamer |
| `audit [file]` | Visual layout check (overflow, fonts, sizing) |
| `compile [file]` | XeLaTeX or Quarto compilation |

**Agents dispatched:**

| Mode | Agents | Output Path |
|------|--------|-------------|
| `create` | Storyteller + storyteller-critic | `paper/talks/[format]_talk.tex` |
| `create --quarto` | Storyteller + storyteller-critic | `paper/quarto/[format]_talk.qmd` |
| `audit` | Visual checks (no agent dispatch) | Advisory report |
| `compile` | Compilation only | Compiled PDF or HTML |

**Output artifacts:**

- Beamer `.tex` or Quarto `.qmd` file.
