...
| `/strategize` | `--pap`, `theory` | Empirical strategy (4 paper types), pre-analysis plan, or formal theory section |
| `/analyze [dataset]` | `--dual [langs]` | End-to-end data analysis (single or parallel language) |
| `/write [section]` | `--humanize`, `style-guide` | Draft sections, strip AI patterns, or extract personal voice from prior papers |
...
| `/checkpoint` | `--auto`, `--memory-only`, `--scaffold-only`, `--dry-run`, `--setup-obsidian` | Session handoff: memory + SESSION_REPORT + research journal (+ Obsidian if configured) |
