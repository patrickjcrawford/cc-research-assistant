...
## Automatic Upgrade (3.x → future versions)

Run `/tools upgrade` inside your project. It fetches the latest infrastructure, replaces `.claude/`, preserves your content, and reports what changed.

::: {.callout-warning}
`/tools upgrade` does **not** work for 2.x → 3.0 because 2.x doesn't have the `/tools` skill. Use the manual upgrade below.
:::

## Manual Upgrade (2.x → 3.0)

::: {.callout-note}
**Your files are safe.** The upgrade only touches the `.claude/` directory (infrastructure). Your paper, scripts, data, bibliography, and all research content are never modified or deleted.
:::

1. **Back up:** `git stash` any uncommitted work
2. **Add upstream:** `git remote add upstream https://github.com/hugosantanna/clo-author.git`
3. **Copy infrastructure:** replace your `.claude/` and `templates/` with the 3.0 versions from upstream
4. **Keep your content:** don't rename folders that already have files in them --- if your paper is in `Paper/` not `paper/`, that's fine
5. **Update CLAUDE.md:** adjust the folder structure section and compilation commands to match your actual layout
