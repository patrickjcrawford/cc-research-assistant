...
**What you type:** `/talk create seminar beamer`
...
**What you see:** A Beamer `.tex` file in `paper/talks/` with speaker notes on every slide, plus a critic review report. Talk scores are advisory --- they do not block commits.

**What to do next:** Review the slides against your paper. Run `/talk audit` to catch text overflow and sizing issues. Run `/talk compile` to build the PDF. For Quarto RevealJS output, add `--quarto`. For all format options, see the [Command Reference](reference.qmd).
