...
Prevents Claude from editing files that should only be changed manually. When Claude attempts an Edit or Write operation, this hook extracts the target file path from the tool input, checks the filename against a list of protected patterns, and blocks the operation if there is a match.
...
## 2. post-edit-lint.sh {#post-edit-lint}

**Event:** `PostToolUse` (matcher: `Edit|Write`) | **Timeout:** 10s | **Blocking:** No (advisory)

### What it does

Runs an automated linter on R, Python, or Julia scripts immediately after Claude edits or writes them. Catches prohibited patterns from the coding standards before the coder-critic reviews.
...
The coder-critic catches these same issues, but it runs later in the pipeline. This hook gives Claude immediate feedback on prohibited patterns (like `setwd()` or missing `set.seed()`) so it can fix them in the same editing pass rather than waiting for a review cycle.

---

## 3. lint-scripts.sh {#lint-scripts}
...
## 4. pre-compact.py {#pre-compact}

**Event:** `PreCompact` | **Timeout:** 10s | **Blocking:** No (message only)

### What it does

Captures the current session state before context compaction destroys it. Context compaction summarizes the conversation history to free up token space, but it can lose important details about what you were working on. This hook preserves that context so it can be restored afterward.
...
## 5. post-compact-restore.py {#post-compact-restore}

**Event:** `SessionStart` (matcher: `compact|resume`) | **Timeout:** 5s | **Blocking:** No (advisory)

### What it does

Restores context after compaction by reading the state saved by `pre-compact.py` and printing it into the new session. This gives Claude immediate awareness of what was happening before compaction.
...
Compaction is the biggest source of context loss in long sessions. Without these hooks, Claude resumes after compaction with no memory of the active plan, current task, or recent decisions. The pre-compact/post-compact pair ensures continuity --- the first hook captures state, the second restores it.

---

## 6. post-merge.sh {#post-merge}
...
- **`PreToolUse` / `PostToolUse`**: the matcher tests against the tool name (`Edit`, `Write`, `Bash`, etc.). `"Edit|Write"` matches either tool.
